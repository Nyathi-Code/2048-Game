...
class Game_Board(Widget):
    ...
    def __init__(self, **kwargs):
        super(Game_Board, self).__init__(**kwargs)
        self.resize()
    ...
    def new_tile(self, *args):
        ...
        tile = Game_Tiles(pos=self.cell_position(x, y),
                    size=self.cell_size)
        self.b[x][y] = tile
    ...
    def allowed_cell(self, board_x, board_y):
        return (board_x >= 0 and board_y >= 0 and
                board_x <= 3 and board_y <= 3)

    def possible_move(self, board_x, board_y):
        return (self.allowed_cell(board_x, board_y) and
                self.b[board_x][board_y] is None)

    def possible_combination(self, board_x, board_y, number):
        return (self.allowed_cell(board_x, board_y) and
                self.b[board_x][board_y] is not None and
                self.b[board_x][board_y].number == number)

    def cell_position(self, board_x, board_y):
    ...
    def move(self, dir_x, dir_y):
        ...
        for board_x, board_y in all_cells(dir_x > 0, dir_y > 0):
            ...
            x, y = board_x, board_y
            while self.possible_move(x + dir_x, y + dir_y):
                self.b[x][y] = None
                x += dir_x
                y += dir_y
                self.b[x][y] = tile

            if self.possible_combination(x + dir_x, y + dir_y, tile.number):
                self.b[x][y] = None
                x += dir_x
                y += dir_y
            ...
            anim = Animation(pos=self.cell_position(x, y),
    ...
    def resize(self, *args):
        ...
        with self.canvas.before:
            ...
            for board_x, board_y in all_cells():
                BorderImage(pos=self.cell_position(board_x, board_y),
        ...
        for board_x, board_y in all_cells():
            tile = self.b[board_x][board_y]
            if tile:
                tile.resize(pos=self.cell_position(board_x, board_y),
                            size=self.cell_size)
...
class Game_Tiles(Widget):
    font_size = NumericProperty(24)
    number = NumericProperty(2)
    color = ListProperty(get_color_from_hex(tile_colors[2]))
    number_color = ListProperty(get_color_from_hex('FFFFF'))
    Window.size = (300, 500)

    def __init__(self, number=2, **kwargs):
        super(Game_Tiles, self).__init__(**kwargs)
